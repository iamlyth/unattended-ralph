...
- History integrity: 34 commits between plan base and HEAD, zero merge
  ...
  `.factory/`, `docs/`, `.factory/tools/`, `.factory/tests/legacy/`, `README.md`, `AGENTS.md`.
...
- Production paths: `.factory/loop/` (state.py, lock.py, launch.py,
  ...
  `.factory/tests/`, `.factory/tools/` (verify-boilerplate.sh, final-gate.sh,
  ...
  credential-guard.py, ollama-usage-guard.sh), `.factory/tests/legacy/`, `docs/`
...
- Executable evidence: `.factory/tools/validate-implementation-plan.py planning .factory/artifacts/implementation-plan.md` BLOCKED (observed exit 0)
- Executable evidence: `.factory/tools/validate-conformance.py planning` BLOCKED (observed exit 0, 24 requirements)
- Executable evidence: `.factory/tools/validate-conformance.py complete` BLOCKED (observed exit 1: AUTH-01 partial)
- Executable evidence: `.factory/tools/validate-blocked-facts.py planning .factory/artifacts/blocked-facts.json` BLOCKED (observed exit 0, 23 facts)
- Executable evidence: `.factory/tools/check-docs-sync.sh` BLOCKED (observed exit 0)
- Executable evidence: `.factory/tools/check-capability-contracts.py` BLOCKED (observed exit 0, 0 contracts)
- Executable evidence: `.factory/tools/check-capability-evidence.py` BLOCKED (observed exit 0, no declared capabilities)
- Executable evidence: `.factory/tools/check-factory-environment.py .factory/environment.toml` BLOCKED (observed exit 0, 0 tools, 0 runners)
- Executable evidence: `.factory/tools/check-golden-policy.py` BLOCKED (observed exit 0)
- Executable evidence: `.factory/tools/check-factory-runner-evidence.py --print-digest` BLOCKED (observed exit 0, digest `0e431408…` stable over repeated runs)
- Executable evidence: `.factory/tools/check-factory-runner-evidence.py --print-capabilities` BLOCKED (observed exit 0, zero evidenced capabilities)
- Executable evidence: `.factory/tools/run-factory-runners.py` BLOCKED (observed exit 0, 0 runner(s) passed)
...
- Executable evidence: `.factory/tools/check-generic-leakage.sh` BLOCKED (observed exit 0)
- Executable evidence: `.factory/tools/verify-boilerplate.sh` BLOCKED (observed exit 0, full gate)
- Executable evidence: `.factory/tools/visual-audit-gate.sh` BLOCKED (observed exit 0, visual audit disabled)
- Executable evidence: `.factory/tools/check-installed-functional-evidence.sh` BLOCKED (observed exit 1: tested commit `61356a0…` not an ancestor of HEAD)
- Executable evidence: `.factory/tools/check-audit-receipts.py` BLOCKED (observed exit 1: zero receipts at base)
- Executable evidence: `.factory/tools/check-campaign-objectives.py` BLOCKED (observed exit 1: round-1 categories uncovered)
- Executable evidence: `.factory/tools/check-plan-freshness.sh` BLOCKED (observed exit 0)
...
- Environment limits: `.factory/environment.toml` declares zero tools and
  ...
  categories `runner-evidence` (`./.factory/tools/run-factory-runners.py`,
  allow_manifest=true) and `project-verify`
  (`./.factory/tools/verify-boilerplate.sh`, allow_manifest=false) — none can be
...
- Production evidence: `.factory/artifacts/conformance.json` classifies
  ...
  `verified`; `.factory/tools/validate-conformance.py complete` exits 1 (AUTH-01
  partial); `.factory/tools/validate-blocked-facts.py complete` exits 1 (FACT-001
...
- Required remediation: complete every row to `verified` with exact-commit
  ...
  then re-run `.factory/tools/validate-conformance.py complete` and
  `.factory/tools/validate-blocked-facts.py complete` to exit 0 before any
...
- Production evidence: `.factory-state/audit-receipts/` is empty (0
  ...
  `.factory/tools/check-audit-receipts.py` exits 1.
- Required remediation: run the trusted receipt wrapper
  (`.factory/tools/machine-receipt.py`) for each installed-tier gate at the audit
  ...
  `.factory/tools/check-audit-receipts.py` exits 0. A model assertion or
...
- Production evidence: `.factory/environment.toml` declares zero
  ...
  capabilities; `.factory/tools/check-factory-runner-evidence.py --print-capabilities`
  returns empty (exit 0); `.factory/tools/run-factory-runners.py` reports 0
...
- Required remediation: a human must provision the hardware runner,
  ...
  accepted by `.factory/tools/check-factory-runner-evidence.py` before RUNNER-01
...
- Production evidence: `.factory/campaign-objectives.json` round 1 selects
  ...
  runners; `.factory/tools/check-campaign-objectives.py --round 1 --base
...
- Production evidence: `.factory-state/installed-functional-evidence.env`
  ...
  `.factory/tools/check-installed-functional-evidence.sh` exits 1 ("tested commit
...
- Required remediation: produce fresh `test_installed_functional` PASS
  ...
  `.factory/tools/check-installed-functional-evidence.sh` to exit 0 with zero
...
- Required remediation: correct the preamble prose to state MIG-01 and
  ...
  sidecar, and re-run `.factory/tools/check-docs-sync.sh`.
...
- `.factory/tools/verify-boilerplate.sh`, the 27-case adversarial suite, the
