...
# ─── Worktree management for parallel developer isolation ─────────────

def _create_worktree(root: Path, name: str, base_commit: str) -> str | None:
    ...
    if result.returncode != 0:
        print(f"  worktree: failed to create {name}: "
...
def run_implementation_phase(
    ...
    # Each developer can edit, build, and test independently.
    ...
    for dev, wt_path in worktrees:
        dev_name = dev.get("name", "unnamed")
        patch = _collect_patch(wt_path, base_commit)
        if patch.strip():
...
def cmd_plan(args, config: dict, env: dict) -> int:
    ...
    # Clean up any stale worktrees from a previous interrupted campaign.
    subprocess.run(
        ["git", "worktree", "prune"],
        cwd=str(ROOT), capture_output=True, timeout=30,
    )
