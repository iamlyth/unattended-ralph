...
class CanonicalPlanSelectionTest(unittest.TestCase):
    ...
    def test_canonical_plan_selects_first_runnable_pending(self) -> None:
        ...
        # Task 7 is complete at this plan revision, so Task 8 is runnable.
        # Task 13 is also runnable from Task 1; numeric priority then
        # lexicographic identifier deterministically picks Task 8.
        self.assertEqual(selection.task_id, 8)
        # Tasks 8 and 13 are the runnable pending tasks; every other unfinished
        # task still has an incomplete dependency.
        ...
        self.assertEqual(runnable, [8, 13])
...
class PurityAndLedgerBoundaryTest(unittest.TestCase):
    ...
    def test_selection_performs_no_io(self) -> None:
        ...
        plan = Plan.from_file(CANONICAL_PLAN)
        self.assertEqual(select_task(plan).task_id, 8)
...
class TrustedCliTest(unittest.TestCase):
    ...
    def test_cli_selects_canonical_next_task(self) -> None:
        ...
        self.assertEqual(result.stdout.strip(), "selected=8")
