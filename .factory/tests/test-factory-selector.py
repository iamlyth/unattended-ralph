...
class CanonicalPlanSelectionTest(unittest.TestCase):
    ...
    def test_canonical_plan_selects_next_runnable_task(self) -> None:
        ...
        # depends only on complete tasks and is the next runnable task.
...
class TrustedCliTest(unittest.TestCase):
    ...
    def test_cli_reports_canonical_next_task(self) -> None:
        result = self._run("select", str(CANONICAL_PLAN))
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertEqual(result.stdout.strip(), "selected=28")
