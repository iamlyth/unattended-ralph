...
RECORDER="$PROJECT_ROOT/.factory/tools/machine-receipt.py"
CHECKER="$PROJECT_ROOT/.factory/tools/check-audit-receipts.py"
...
setup_repo() {
    ...
    cp "$RECORDER" "$CHECKER" "$RUNNER_EVIDENCE" "$ENV_CHECKER" \
       "$PROJECT_ROOT/.factory/tools/factory_runner_artifacts.py" "$dir/.factory/tools/"
...
(cd "$tmp/audit" && env -u FACTORY_CAMPAIGN_AUDIT_ROUND -u FACTORY_CAMPAIGN_AUDIT_BASE -u FACTORY_CAMPAIGN_AUDIT_NONCE \
    ./.factory/tools/machine-receipt.py --tag bare -- true >/dev/null 2>&1)
bare_rc=$?
set -e
...
(cd "$tmp/audit" && ./.factory/tools/machine-receipt.py --tag probe \
...
(cd "$tmp/audit" && ./.factory/tools/check-audit-receipts.py >/dev/null)
...
must_fail "fabricated command prose" \
    "cd '$tmp/audit' && ./.factory/tools/check-audit-receipts.py"
...
(cd "$tmp/audit" && ./.factory/tools/machine-receipt.py --tag pass-again \
    --audit-round "$ROUND" --evidence-commit "$head" --nonce "$(printf 'a%.0s' {1..64})" \
    -- true >/dev/null)
...
(cd "$tmp/audit" && ./.factory/tools/machine-receipt.py --tag tampered \
    --audit-round "$ROUND" --evidence-commit "$head" --nonce "$(printf 'a%.0s' {1..64})" \
    -- true >/dev/null)
...
must_fail "unsigned runner manifest" \
    "cd '$tmp/unsigned' && ./.factory/tools/check-audit-receipts.py"
...
set +e
(cd "$tmp/audit" && env -u FACTORY_CAMPAIGN_AUDIT_ROUND -u FACTORY_CAMPAIGN_AUDIT_BASE -u FACTORY_CAMPAIGN_AUDIT_NONCE \
    ./.factory/tools/check-audit-receipts.py >/dev/null 2>&1)
no_base_rc=$?
set -e
...
(cd "$tmp/audit" && ./.factory/tools/machine-receipt.py --tag fork-during-term \
...
(cd "$tmp/audit" && ./.factory/tools/machine-receipt.py --tag foreign-untouched \
    --audit-round "$ROUND" --evidence-commit "$head" \
    --nonce "$(printf 'a%.0s' {1..64})" \
    -- true >/dev/null)
