...
CHECKER="$PROJECT_ROOT/.factory/tools/check-campaign-objectives.py"
RUNNER_EVIDENCE="$PROJECT_ROOT/.factory/tools/check-factory-runner-evidence.py"
ENV_CHECKER="$PROJECT_ROOT/.factory/tools/check-factory-environment.py"
...
setup_repo() {
    ...
    cp "$CHECKER" "$RUNNER_EVIDENCE" "$ENV_CHECKER" "$dir/.factory/tools/"
    ...
    cat > "$dir/.factory/capability-contracts.json" <<'CONTRACTS'
{
  ...
  "capabilities": [
    {
      "name": "runner-gate",
      "probe_argv": ["./.factory/tools/run-factory-runners.py"],
      "probe_marker": "",
      "must_execute": true,
      "must_not_skip": [],
    ...
    {
      "name": "system-gate",
      "probe_argv": ["./.factory/tools/probe-system.sh"],
      "probe_marker": "",
      "must_execute": true,
      "must_not_skip": [],
      "deny_simulated_markers": []
    ...
    {
      "name": "verify-gate",
      "probe_argv": ["./.factory/tools/verify-project.sh"],
      "probe_marker": "",
      "must_execute": true,
      "must_not_skip": [],
      "deny_simulated_markers": []
    ...
    cat > "$dir/.factory/campaign-receipt-policy.json" <<'POLICY'
{
  ...
  "categories": [
    {"name": "runner-evidence", "tier": "installed", "allow_manifest": true, "argv": [["./.factory/tools/run-factory-runners.py"]]},
    {"name": "project-verify", "tier": "installed", "allow_manifest": false, "argv": [["./.factory/tools/verify-project.sh"]]},
    {"name": "installed-visual", "tier": "installed", "allow_manifest": false, "argv": [["./.factory/tools/probe-visual.sh"]]},
    {"name": "visual-render", "tier": "installed", "allow_manifest": false, "argv": [["./.factory/tools/probe-visual.sh"]]},
    {"name": "real-system-service", "tier": "real_system", "allow_manifest": true, "argv": [["./.factory/tools/probe-system.sh"]]},
    {"name": "external-observer", "tier": "real_system", "allow_manifest": true, "argv": [["./.factory/tools/probe-system.sh"]]}
  ]
}
POLICY
...
- Executable evidence: ./.factory/tools/run-factory-runners.py PASS [receipt: .factory-state/audit-receipts/runner-evidence.json]
- Executable evidence: ./.factory/tools/verify-project.sh PASS [receipt: .factory-state/audit-receipts/project-verify.json]
- Executable evidence: ./.factory/tools/probe-visual.sh FAIL [receipt: .factory-state/audit-receipts/visual-render.json]
...
expect_rc() {
    ...
    set +e
    (cd "$dir" && ./.factory/tools/check-campaign-objectives.py --round "$round" --base "$base" >/dev/null 2>&1)
    local rc=$?
    set -e
    [[ $rc -eq $expected ]] || {
...
receipt "$tmp/blessed" 1 runner-evidence 0 ./.factory/tools/run-factory-runners.py
receipt "$tmp/blessed" 1 project-verify 0 ./.factory/tools/verify-project.sh
receipt "$tmp/blessed" 1 visual-render 1 ./.factory/tools/probe-visual.sh
...
receipt "$tmp/blessed" 3 real-system-service 0 ./.factory/tools/probe-system.sh
receipt "$tmp/blessed" 3 external-observer 0 ./.factory/tools/probe-system.sh
...
- Executable evidence: ./.factory/tools/probe-system.sh PASS [receipt: .factory-state/audit-receipts/real-system-service.json]
- Executable evidence: ./.factory/tools/probe-system.sh PASS [receipt: .factory-state/audit-receipts/external-observer.json]
...
# Campaign Audit
## Evidence reviewed
- Executable evidence: ./.factory/tools/probe.sh BLOCKED no real system service available
- Executable evidence: ./.factory/tools/probe-system.sh PASS [receipt: .factory-state/audit-receipts/external-observer.json]
...
cat > "$tmp/blessed/.factory-state/audit-receipts/project-verify.json" <<JSON
{"schema": "ralph-audit-receipt/v1", "tag": "project-verify", "argv": ["./.factory/tools/verify-project.sh"], "argv_sha256": "$(printf 'e%.0s' {1..64})", "exit_code": 0, "stdout_sha256": "$(printf 'c%.0s' {1..64})", "stderr_sha256": "$(printf 'd%.0s' {1..64})", "started_at": 1, "finished_at": 2, "evidence_commit": "$(printf '2%.0s' {1..40})", "coordinator_round": 1, "coordinator_nonce": "$NONCE"}
JSON
cat > "$tmp/blessed/.factory/artifacts/campaign-audit.md" <<'REPORT'
---
result: pass
...
# Campaign Audit
## Evidence reviewed
- Executable evidence: ./.factory/tools/probe.sh PASS [manifest: .factory-state/runner-manifests/runner-evidence/manifest.json]
- Executable evidence: ./.factory/tools/verify-project.sh PASS [receipt: .factory-state/audit-receipts/project-verify.json]
...
## Evidence reviewed
- Executable evidence: ./.factory/tools/verify-project.sh PASS [receipt: .factory-state/audit-receipts/project-verify.json]
- Executable evidence: ./.factory/tools/probe.sh PASS [manifest: .factory-state/runner-evidence/fake-runner/$head/manifest.json]
...
## Evidence reviewed
- Executable evidence: ./.factory/tools/verify-project.sh PASS [receipt: .factory-state/audit-receipts/project-verify.json]
- Executable evidence: ./.factory/tools/probe.sh PASS [manifest: .factory-state/runner-evidence/real-system-service/$head/manifest.json]
...
# Campaign Audit
## Evidence reviewed
- Executable evidence: ./.factory/tools/probe.sh PASS [manifest: .factory-state/runner-evidence/verify-runner/$head/manifest.json]
...
(cd "$tmp/blessed" && env -u FACTORY_CAMPAIGN_AUDIT_ROUND \
    ./.factory/tools/check-campaign-objectives.py --base "$head" >/dev/null 2>&1)
env_missing_rc=$?
set -e
