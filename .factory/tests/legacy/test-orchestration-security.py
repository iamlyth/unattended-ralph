...
def copy_scripts(root: Path, *names: str) -> None:
    ...
    for name in names:
        shutil.copy2(SOURCE / "scripts" / name, root / ".factory" / "tools" / name)
...
def test_symlink_safe_state_markers() -> None:
    ...
    try:
        ...
        helper = root / ".factory/tools/factory-state-file.py"
...
def test_state_removal_quarantines_post_check_substitution() -> None:
    module = load_module(SOURCE / ".factory/tools/factory_state_io.py", "factory_state_io_race_module")
...
def test_loop_lock_process_identity_and_inode_race() -> None:
    module = load_module(SOURCE / ".factory/tools/ralph_lock.py", "ralph_lock_test_module")
...
def test_receiver_side_token_contamination() -> None:
    ...
    try:
        ...
        helper = root / ".factory/tools/ralph-event-boundary.py"
...
def test_verifier_executable_blob_binding() -> None:
    ...
    try:
        ...
        (root / ".factory/config.toml").write_text(
            '[verification]\ncampaign_command = ["./.factory/tools/verify-project.sh", "--strict"]\n',
            encoding="utf-8",
        )
        ...
        run(["git", "restore", "--", ".factory/tools/verify-project.sh"], root)
...
def test_retained_helper_fd_immune_to_pathname_swap() -> None:
    ...
    try:
        ...
        try:
            ...
            helper.rename(root / ".factory/tools/campaign-verifier-binding.py.original")
        ...
        assert binding["helper"] == {
            "path": ".factory/tools/campaign-verifier-binding.py",
...
def test_verifier_swap_in_final_exec_race_runs_original() -> None:
    ...
    try:
        ...
        run(["git", "add", "."], root)
        run(["git", "commit", "-qm", "base"], root)
        driver = root / ".factory/tools/race-driver.py"
        driver.write_text(
            """import importlib.util, os, sys
from pathlib import Path
root = Path(sys.argv[1])
spec = importlib.util.spec_from_file_location('cvb', root / '.factory/tools/campaign-verifier-binding.py')
...
def racing_execve(path, argv, env):
    ...
    verifier.rename(root / '.factory/tools/verify-project.sh.original')
    substitute = root / '.factory/tools/verify-project.sh'
...
def test_one_time_supervision_migration() -> None:
    ...
    try:
        ...
        helper = root / ".factory/tools/ralph-supervision-migrate.py"
        ...
        continuation = run(
            [
                "bash", "-c",
                "source .factory/tools/ralph-supervision.sh; "
        ...
        run(
            [
                str(root / ".factory/tools/ralph-campaign-state.py"),
        ...
        run([str(root / ".factory/tools/ralph-campaign-state.py"), "show"], root)
    finally:
        shutil.rmtree(root)
