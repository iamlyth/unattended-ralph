...
cp "$PROJECT_ROOT/.factory/tools/initialize-campaign-audit.py" \
   ...
   "$PROJECT_ROOT/.factory/tools/run-factory-runners.py" \
   "$PROJECT_ROOT/.factory/tools/check-factory-runner-evidence.py" \
   "$PROJECT_ROOT/.factory/tools/check-factory-environment.py" \
   "$PROJECT_ROOT/.factory/tools/factory_runner_artifacts.py" "$tmp/scripts/"
mkdir -p "$tmp/.factory/loop"
...
(cd "$tmp" && ./.factory/tools/run-factory-runners.py >/dev/null)
runner_digest=$(cd "$tmp" && ./.factory/tools/check-factory-runner-evidence.py --expected-commit "$base" --expected-campaign-id "$FACTORY_CAMPAIGN_ID" --expected-readiness-nonce "$FACTORY_READINESS_NONCE" --print-digest)
(cd "$tmp" && ./.factory/tools/initialize-campaign-audit.py --round 1 --base "$base" \
...
(cd "$tmp" && ./.factory/tools/validate-campaign-audit.py complete --expected-round 1 --expected-base "$base" \
    --expected-runner-evidence-sha256 "$runner_digest" >/dev/null)
set +e
(cd "$tmp" && ./.factory/tools/validate-campaign-audit.py complete --expected-round 2 --expected-base "$base" \
