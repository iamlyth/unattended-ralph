...
cp "$source_root/scripts/check-installed-functional-evidence.sh" .factory/tools/
cp "$source_root/scripts/machine-receipt.py" .factory/tools/
...
./.factory/tools/machine-receipt.py --root "$PWD" --tag installed-harness-smoke \
...
./.factory/tools/check-installed-functional-evidence.sh >/dev/null
...
printf 'changed\n' >> src/app.c
if ./.factory/tools/check-installed-functional-evidence.sh >/dev/null 2>&1; then
...
PY
if ./.factory/tools/check-installed-functional-evidence.sh >/dev/null 2>&1 || [[ -e "$sentinel" ]]; then
