...
setup_repo() {
    ...
    cp "$CONTRACT_CHECKER" "$EVIDENCE_CHECKER" "$dir/.factory/tools/"
    cp "$PROJECT_ROOT/.factory/loop/gitutil.py" "$dir/.factory/loop/"
    chmod +x "$dir/.factory/tools/"*.py
    printf '#!/usr/bin/env bash\nexit 0\n' > "$dir/.factory/tools/verify-project.sh"
    chmod +x "$dir/.factory/tools/verify-project.sh"
...
verify_argv = ["./.factory/tools/verify-project.sh"]
EOF
...
must_fail "contract claiming an undeclared capability" \
    "cd '$tmp/undeclared-contract' && ./.factory/tools/check-capability-contracts.py"
...
must_fail "declared capability without a contract" \
    "cd '$tmp/missing-contract' && ./.factory/tools/check-capability-contracts.py"
...
(cd "$tmp/valid" && ./.factory/tools/check-capability-contracts.py >/dev/null)
must_fail "legacy unnamespaced receipt is not evidence" \
    "cd '$tmp/valid' && ./.factory/tools/check-capability-evidence.py"
...
PY
(cd "$tmp/runner-class-ok" && ./.factory/tools/check-capability-contracts.py >/dev/null)
...
PY
    must_fail "invalid runner_class '$bad'" \
        "cd '$tmp/runner-class-bad-$runner_class_bad' && ./.factory/tools/check-capability-contracts.py"
done
...
data = json.load(open(path, encoding="utf-8"))
data['capabilities'][0]['probe_argv'] = ["nix-shell", "--run", "bash .factory/tools/probe.sh", token]
open(path, 'w', encoding="utf-8").write(json.dumps(data, indent=2))
PY
    must_fail "probe argv fixture token '$token'" \
        "cd '$tmp/fixture-token-$fixture_token_bad' && ./.factory/tools/check-capability-contracts.py"
done
...
must_fail "missing runner receipt" \
    "cd '$tmp/missing-receipt' && ./.factory/tools/check-capability-evidence.py"
...
must_fail "skipped probe in the receipt" \
    "cd '$tmp/skipped-probe' && ./.factory/tools/check-capability-evidence.py"
...
must_fail "simulated marker in the receipt" \
    "cd '$tmp/simulated' && ./.factory/tools/check-capability-evidence.py"
...
must_fail "receipt without the probe marker" \
    "cd '$tmp/missing-marker' && ./.factory/tools/check-capability-evidence.py"
...
must_fail "stale receipt not bound to HEAD" \
    "cd '$tmp/stale' && ./.factory/tools/check-capability-evidence.py"
...
must_fail "failed runner manifest" \
    "cd '$tmp/fail-manifest' && ./.factory/tools/check-capability-evidence.py"

# An explicit empty capability request must not pass a gate.
must_fail "empty explicit capability set" \
    "cd '$tmp/valid' && ./.factory/tools/check-capability-evidence.py --capabilities ''"
