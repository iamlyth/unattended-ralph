...
class RunnerFrameworkTests(unittest.TestCase):
 ...
 def test_signer_has_no_direct_oracle(self):
  result=subprocess.run([sys.executable,str(ROOT/".factory/tools/factory-runner-signer.py"),"--broker-fd","-1"],input=b'{}\n',capture_output=True)
 ...
 def test_forced_command_and_bootstrap_are_exact(self):
  server=(ROOT/".factory/tools/factory-runner-server.py").read_text();installer=(ROOT/".factory/tools/install-factory-runner-v2.sh").read_text();bootstrap=(ROOT/".factory/tools/factory-runner-root-bootstrap").read_text()
 ...
 def test_install_manifest_rejects_dirty_and_symlink_tree(self):
  with tempfile.TemporaryDirectory() as td:
   ...
   r=subprocess.run([sys.executable,str(ROOT/".factory/tools/generate-runner-install-manifest.py"),"--source",str(repo),"--output",str(repo/"m.json")],capture_output=True);self.assertNotEqual(r.returncode,0)
 def test_archive_adapter_namespaces_runner_evidence(self):
  text=(ROOT/".factory/tools/archive-factory-campaign.py").read_text();self.assertIn('runner-evidence/',text);self.assertIn('PurePosixPath',text)
  archive=module("fixture_archive",ROOT/".factory/tools/archive-factory-campaign.py")
  with tempfile.TemporaryDirectory() as td:
 ...
 def test_bounds_and_nonce_controls_are_present(self):
  broker=(ROOT/".factory/tools/factory-runner-broker.py").read_text()
 ...
 def test_fork_setsid_output_disk_and_nonce_flood_controls(self):
  text=(ROOT/".factory/tools/factory-runner-broker.py").read_text()
  ...
  broker=module("fixture_broker",ROOT/".factory/tools/factory-runner-broker.py")
 ...
 def test_pin_substitution_and_publication_race_controls(self):
  broker=(ROOT/".factory/tools/factory-runner-broker.py").read_text();client=(ROOT/".factory/tools/run-factory-runners.py").read_text()
  self.assertIn("executable pin substitution",broker);self.assertIn("os.O_NOFOLLOW",broker)
  self.assertIn("/proc/self/fd/",broker);self.assertIn("executable pin ancestry is mutable",broker);self.assertIn("pass_fds=_pass_fds",broker)
