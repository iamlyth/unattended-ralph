...
for script in visual-audit-provenance.py visual-audit-review.py visual-audit-lease.py \
    ...
    cp "$PROJECT_ROOT/.factory/tools/$script" "$tmp/scripts/" 2>/dev/null || true
done
...
set +e
"$PROJECT_ROOT/.factory/tools/visual-capture-driver.sh" state-primary "$tmp/cap.png" "$HEAD" >"$tmp/driver.out" 2>&1
rc=$?
set -e
...
sed -e 's/enabled = false/enabled = true/' \
    -e 's|vision_model = ""|vision_model = "test-model"|' \
    -e 's|sdk_driver = ".factory/tools/visual-audit-review-sdk.mjs"|sdk_driver = ".factory/tools/counting-driver.sh"|' \
    "$tmp/.factory/visual-audit.toml" > "$tmp/gate-config.toml"
mv "$tmp/gate-config.toml" "$tmp/.factory/visual-audit.toml"
git add .factory/visual-audit.toml .factory/tools/counting-driver.sh .factory/tools/mock-driver.sh
