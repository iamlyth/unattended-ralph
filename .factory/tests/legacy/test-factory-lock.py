...
check=subprocess.run([str(root/'.factory/tools/factory-lock-exec.py'),str(root),'--check'],capture_output=True)
assert check.returncode != 0
contender=subprocess.run([str(root/'.factory/tools/factory-lock-exec.py'),str(root),'--','true'],capture_output=True)
...
if pid==0:
    fcntl.flock(separate, fcntl.LOCK_UN)
    state=subprocess.run([str(root/'.factory/tools/ralph-campaign-state.py'),'show'],capture_output=True)
...
""",
        ...
        probe.chmod(0o755)
        result = run(
            [str(root / ".factory/tools/factory-lock-exec.py"), str(root), "--", str(probe), str(root)],
            root,
        )
        ...
        run([str(root / ".factory/tools/factory-lock-exec.py"), str(root), "--", "true"], root)
...
def test_legacy_holder_and_unsafe_legacy_fail_closed() -> None:
    root = make_repo()
    try:
        helper = root / ".factory/tools/factory-lock-exec.py"
        legacy = root / ".factory-lock"
...
def test_legacy_substitution_after_final_check_is_not_unlinked() -> None:
    root = make_repo()
    try:
        spec = importlib.util.spec_from_file_location("factory_lock_race", root / ".factory/tools/factory_lock.py")
...
""",
        ...
        probe.chmod(0o755)
        result = run(
            [str(root / ".factory/tools/factory-lock-exec.py"), str(root), "--",
             str(probe), str(root)],
            root,
            env=env,
        )
