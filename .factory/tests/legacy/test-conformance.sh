...
VALIDATOR="$PROJECT_ROOT/.factory/tools/validate-conformance.py"
...
FACTS_VALIDATOR="$PROJECT_ROOT/.factory/tools/validate-blocked-facts.py"
...
setup_repo() {
    ...
    cp "$VALIDATOR" "$EVIDENCE_CHECKER" "$CONTRACT_CHECKER" "$FACTS_VALIDATOR" "$dir/.factory/tools/"
    ...
    printf '#!/usr/bin/env bash\nexit 0\n' > "$dir/.factory/tools/verify-boilerplate.sh"
    chmod +x "$dir/.factory/tools/verify-boilerplate.sh"
...
CONTRACT
    ...
    git -C "$dir" add .factory/tests/legacy/probe.c .factory/tests/legacy/fixtures/runner-manifest.json
  ...
  "requirements": [
    {
      ...
      "artifacts": [".factory/tests/legacy/probe.c"],
      "fact_refs": [],
      "reason": ""
    },
    {
...
for req in data['requirements']:
    ...
    req['receipts'] = [".factory/tests/legacy/fixtures/runner-manifest.json"]
    req['artifacts'] = [".factory/tests/legacy/probe.c"]
    req['fact_refs'] = []
    req['reason'] = ''
if mode == 'free-text':
    for req in data['requirements']:
    ...
    for req in data['requirements']:
        if req['id'] == 'REQ-01':
            req['receipts'] = ['.factory/tests/legacy/fixtures/missing-receipt.json']
elif mode == 'uncommitted-ref':
    # The ref exists only in the working tree, never as a blob at the commit.
    for req in data['requirements']:
        if req['id'] == 'REQ-01':
            req['receipts'] = ['.factory/tests/legacy/fixtures/working-tree-only.json']
    ...
    for req in data['requirements']:
        if req['id'] == 'REQ-01':
            req['artifacts'] = ['.factory/tests/legacy/../../escape']
    ...
    for req in data['requirements']:
        if req['id'] == 'REQ-01':
            req['artifacts'] = ['.factory/tests/legacy/fixtures/working-tree-only.json']
...
expect_fail() {
    local dir=$1 label=$2
    set +e
    (cd "$dir" && ./.factory/tools/validate-conformance.py complete .factory/artifacts/conformance.json >/dev/null 2>&1)
    local rc=$?
    set -e
...
(cd "$tmp/blessed" && ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json >/dev/null)
(cd "$tmp/blessed" && ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json --facts .factory/artifacts/blocked-facts.json >/dev/null)
(cd "$tmp/blessed" && ./.factory/tools/validate-blocked-facts.py planning .factory/artifacts/blocked-facts.json >/dev/null)
(cd "$tmp/blessed" && ./.factory/tools/check-capability-contracts.py >/dev/null)
if (cd "$tmp/blessed" && ./.factory/tools/check-capability-evidence.py >/dev/null 2>&1); then
...
set +e
(cd "$tmp/blessed" && ./.factory/tools/validate-conformance.py complete .factory/artifacts/conformance.json >/dev/null 2>&1)
blocked_complete_rc=$?
set -e
[[ $blocked_complete_rc -eq 1 ]]
set +e
(cd "$tmp/blessed" && ./.factory/tools/validate-blocked-facts.py complete .factory/artifacts/blocked-facts.json >/dev/null 2>&1)
...
set +e
(cd "$tmp/tier-drift" && ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json >/dev/null 2>&1)
tier_drift_rc=$?
set -e
...
set +e
(cd "$tmp/policy-missing" && ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json >/dev/null 2>&1)
policy_missing_rc=$?
set -e
...
set +e
(cd "$tmp/na-misuse" && ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json >/dev/null 2>&1)
na_bad_rc=$?
set -e
...
open(path, 'w').write(json.dumps(data))
PY
(cd "$tmp/na-misuse" && ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json >/dev/null)
...
set +e
(cd "$tmp/na-misuse" && ./.factory/tools/validate-conformance.py complete .factory/artifacts/conformance.json >/dev/null 2>&1)
na_complete_rc=$?
set -e
...
PY
set +e
(cd "$tmp/mismatch" && ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json >/dev/null 2>&1)
mismatch_rc=$?
set -e
...
PY
set +e
(cd "$tmp/unknown-fact" && ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json >/dev/null 2>&1)
unknown_fact_rc=$?
set -e
...
PY
set +e
(cd "$tmp/no-fact-ref" && ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json >/dev/null 2>&1)
no_fact_rc=$?
set -e
...
PY
set +e
(cd "$tmp/verified-fact" && ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json >/dev/null 2>&1)
verified_fact_rc=$?
set -e
...
PY
set +e
(cd "$tmp/fact-drift" && ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json >/dev/null 2>&1)
fact_drift_rc=$?
set -e
...
for fact in data['facts']:
    if fact['id'] == 'FACT-001':
        ...
        fact['resolution'] = {
            'type': 'receipt',
            'refs': ['.factory/tests/legacy/fixtures/runner-manifest.json'],
            'evidence_commit': head,
...
PY
set +e
(cd "$tmp/resolved-referenced" && ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json >/dev/null 2>&1)
resolved_rc=$?
set -e
...
expect_planning_fail() {
    local dir=$1 label=$2
    set +e
    (cd "$dir" && ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json >/dev/null 2>&1)
    local rc=$?
    set -e
...
for label in sidecar policy registry facts contracts; do
    ...
    case "$label" in
        sidecar|policy|registry)
            set +e
            (cd "$tmp/dup-key-$label" && ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json >/dev/null 2>&1)
            dup_rc=$?
            set -e
            ;;
        facts)
            set +e
            (cd "$tmp/dup-key-facts" && ./.factory/tools/validate-blocked-facts.py planning .factory/artifacts/blocked-facts.json >/dev/null 2>&1)
            dup_rc=$?
            set -e
            ;;
        contracts)
            set +e
            (cd "$tmp/dup-key-contracts" && ./.factory/tools/check-capability-contracts.py >/dev/null 2>&1)
            dup_rc=$?
            set -e
            ;;
    esac
...
PY
set +e
(cd "$tmp/dup-id-facts" && ./.factory/tools/validate-blocked-facts.py planning .factory/artifacts/blocked-facts.json >/dev/null 2>&1)
dup_facts_rc=$?
set -e
...
PY
set +e
(cd "$tmp/dup-id-contracts" && ./.factory/tools/check-capability-contracts.py >/dev/null 2>&1)
dup_contracts_rc=$?
set -e
...
(cd "$tmp/blessed" && env \
    ...
    "$PYTHON_BIN" ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json >/dev/null 2>&1)
hardened_env_rc=$?
set -e
...
(cd "$tmp/replace-ref" && ./.factory/tools/validate-conformance.py planning .factory/artifacts/conformance.json >/dev/null)
set +e
