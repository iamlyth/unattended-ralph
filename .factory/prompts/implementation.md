...
7. Update nearby documentation as behavior changes and capture why the implementation and tests matter. Record concise evidence in the plan, including the exact command, result, production path exercised, and semantic outcome—not merely compilation or event consumption. The machine-readable `.factory/artifacts/conformance.json` sidecar is the authority for every `verified` conformance claim: each requirement declares its classification, evidence tier, required capabilities, the exact evidence commit, and receipt/artifact refs. A `verified` normative real-system/visual/consumer row cannot be satisfied by a lower evidence tier or by an undeclared/unevidenced capability; `blocked` and `not_applicable` rows need an explicit reason (spec-scoped for not_applicable), and `blocked` rows fail implementation completion.Any requirement whose evidence is unavailable (undeclared capability, missing system service, missing hardware target, open product defect, or pending human decision) must be recorded as an open fact in `.factory/artifacts/blocked-facts.json` and referenced by the blocked/partial sidecar row; completion requires every fact resolved by an exact receipt/artifact or an explicit human decision where the specification permits it—documentation alone can never resolve one. Never write completion prose into `.factory/artifacts/context-summary.md`; the fresh context receives only the active task, open tasks, unresolved facts, blocked/partial rows, and exact receipt refs, validated by `scripts/check-context-summary.py`. Golden baselines are protected by `.factory/golden-policy.json`: never regenerate or overwrite active goldens; a reviewed `.factory/golden-review.json` manifest (before/after hashes plus human reviewer) must accompany any baseline change and `scripts/check-golden-policy.py` must pass. If any test fails, including one apparently unrelated to the task, investigate it; fix it when safe or append a remediation task rather than dismissing it.
