...
Verify claimed environment evidence using only declared and mechanically evidenced capabilities. A missing backend, GPU, compositor, physical or kernel-backed controller, target consumer, package installation, system service, or hardware runner is not a skip and must not be inferred from synthetic evidence. Coordinator-executed commands are runtime evidence only when recorded by `./scripts/machine-receipt.py --tag <tag> -- <argv...>` and cited as `[receipt: ...]` (or `[manifest: ...]` for an accepted runner receipt); prose cannot certify runtime. Receipt minting is coordinator-bounded: the audit coordinator exports the protected launch binding (`FACTORY_CAMPAIGN_AUDIT_ROUND`, `FACTORY_CAMPAIGN_AUDIT_BASE`, `FACTORY_CAMPAIGN_AUDIT_NONCE`) so a bare model call to `machine-receipt.py` fails. Every receipt records `evidence_commit` equal to the campaign audit base and the coordinator round/nonce binding; stale receipts and receipts reused across rounds are rejected by `scripts/check-audit-receipts.py`. Each objective receipt category is authorized by `.factory/campaign-receipt-policy.json`, which binds the category to an allowlisted argv and evidence tier: a receipt covers a category only when its recorded argv equals the allowlisted argv exactly — arbitrary commands (including a bare `true`), tag/path spoofing, and stale round receipts are rejected. Every `verified` conformance claim must have a matching machine-readable entry in `.factory/artifacts/conformance.json` with an adequate evidence tier and declared/evidenced capabilities; `required_tier` is assigned by the separate `.factory/requirement-policy.json`, never self-declared by the sidecar, and verified refs must exist as Git blobs at the declared evidence commit. Missing contracts, missing receipts, skipped probes, or simulated markers make a capability unevidenced and never auto-reclassified. Human-tier claims and golden approval are out-of-band and non-automatable: never accept agent-authored `human: true`, reviewer strings, or environment reviewer identity — force those cases to findings. Runner manifests certify nothing until a signer is provisioned in `.factory/signer-trust.json` and the manifest carries a valid signature. Any BLOCKED evidence forces `result: findings`; a report with BLOCKED content may never claim `result: pass`. Every round is bound to one falsification objective from `.factory/campaign-objectives.json`: the round cannot pass without machine receipts (or accepted runner manifests) covering that objective's receipt categories, enforced by `scripts/check-campaign-objectives.py`; replaying the same generic suite cannot satisfy all rounds. Unavailable evidence that blocks conformance must be traceable to an open fact in `.factory/artifacts/blocked-facts.json`, and no conformance completion claim is valid while any fact stays open.
...
- **Resource-path injection**: A test that sets an environment variable (e.g., PRODUCT_ICON_DIR) or passes a source-tree path (e.g., data/icons/svg/) to make an image, SVG, font, or texture load is a production-path bypass. The production path must load the resource without the injection. Verify the production path (e.g., product_icon_dir() → resource loader) actually finds the resource in the installed layout; if it does not, flag it as a finding and require the production path to be fixed so the test can exercise it directly.
...
- **Installed-path verification**: The installed functional test must exercise the real production path (e.g., product_icon_dir()), not a source-tree override. If the installed test uses a source-tree path to make resources load, it does not verify the installed product and is a finding.
