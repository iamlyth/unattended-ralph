...
(( $# == 0 )) || { echo "Usage: .factory/tools/check-maintenance-freshness.sh [--planning]" >&2; exit 2; }
cd -- "$PROJECT_ROOT"
...
./.factory/tools/bug-ledger.py validate >/dev/null
./.factory/tools/validate-maintenance-plan.py current "$PLAN" >/dev/null
...
module_path = pathlib.Path('.factory/tools/validate-maintenance-plan.py')
...
ACTUAL_FINGERPRINT=$(./.factory/tools/bug-ledger.py fingerprint "$BUG_ID")
...
if [[ "${BUG_META[0]}" == .factory/bugs/closed.md ]]; then
    ./.factory/tools/validate-maintenance-plan.py complete "$PLAN" >/dev/null || {
