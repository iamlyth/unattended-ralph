...
record = json.loads(subprocess.check_output(
    ['./.factory/tools/bug-ledger.py', 'show', sys.argv[1]], text=True,
).split('\nfingerprint:', 1)[0])
print(record['status'])
PY
)
case "$status" in
    planned) ;;
    triaged)
        ./.factory/tools/bug-ledger.py set-status "$bug_id" planned
        payload=$(printf '{"loop":{"workspace":"%s","id":"maintenance-planning-ledger"},"iteration":{"current":"planned"}}' "$PROJECT_ROOT")
        printf '%s' "$payload" | ./.factory/tools/git-commit-hook.sh --maintenance-ledger
        ;;
    *)
...
./.factory/tools/check-maintenance-freshness.sh >/dev/null
