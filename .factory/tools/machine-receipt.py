...
`.factory/tools/check-audit-receipts.py` requires a matching receipt (exit 0 for PASS)
...
- inside a campaign audit the coordinator exports the protected launch
  ...
  (`.factory/tools/initialize-campaign-audit.py`) and never by the model;
...
Usage:
  .factory/tools/machine-receipt.py --tag <tag> -- <argv...>
...
def coordinator_binding(root: Path, round_number: int | None, evidence_commit: str | None,
    ...
    trusted audit coordinator mints (``.factory/tools/initialize-campaign-audit.py``),
...
def _load_lock_supervision(root: Path):
    ...
    """
    candidates = [
        Path(__file__).resolve().parents[2] / ".factory" / "loop",
