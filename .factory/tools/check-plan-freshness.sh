...
# `--planning` base comparison reads the frozen `.factory/tools/ralph-plan.sh` marker
...
# surface path and every `.factory/tools/` policy/checker/receipt authority must be
...
(( $# == 0 )) || { echo "Usage: .factory/tools/check-plan-freshness.sh [--planning]" >&2; exit 2; }
cd -- "$PROJECT_ROOT"
...
if [[ "$PHASE" == planning ]]; then
    ...
    # frozen `.factory/tools/ralph-plan.sh` marker (`FACTORY_PLANNING_BASE_COMMIT` or
