...
usage() {
    cat <<'EOF'
Usage: .factory/tools/ollama-usage-guard.sh [--check|--wait] [--json] [--html-file FILE]
...
check_once() {
    ...
    if (( rc == 2 )); then
        if [[ -n "$HTML_FILE" ]]; then
            ...
            echo "ollama-guard: OLLAMA_COOKIE is missing; run source .factory/tools/update-ollama-cookies.sh" >&2
        fi
        return 2
    elif (( rc != 0 )); then
    ...
    case "$classification" in
        auth)
            echo "ollama-guard: authentication expired; refresh cookies with source .factory/tools/update-ollama-cookies.sh" >&2
            return 2
            ;;
        parse)
