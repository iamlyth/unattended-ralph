...
PY
    ...
    ./.factory/tools/check-scratchpad.sh "$token"
else
...
if git diff --cached --quiet; then
    if [[ "$FINAL_HANDOFF" == true ]]; then
        ...
        ./.factory/tools/ralph-final-state.py ensure-checkpoint "$MODE" "$(git rev-parse HEAD)" >/dev/null
    fi
    exit 0
fi
...
if [[ "$FINAL_HANDOFF" == true ]]; then
    ...
    ./.factory/tools/ralph-final-state.py allow-checkpoint-commit "$MODE" "$(git rev-parse HEAD)" >/dev/null
fi
