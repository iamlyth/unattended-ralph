...
case "$MODE" in
    --planning)
        ./.factory/tools/plan-scope-guard.sh
        ./.factory/tools/check-plan-freshness.sh --planning
        ./.factory/tools/check-scratchpad.sh PLAN_COMPLETE
        ...
        if [[ -f .factory/artifacts/blocked-facts.json ]]; then
            ./.factory/tools/validate-blocked-facts.py planning .factory/artifacts/blocked-facts.json
        fi
    ...
    --maintenance-planning)
        ./.factory/tools/maintenance-plan-scope-guard.sh
        ...
        ./.factory/tools/check-scratchpad.sh MAINTENANCE_PLAN_COMPLETE
        echo "final-gate: maintenance planning completion accepted"
        ;;
    --implementation)
        ./.factory/tools/check-plan-freshness.sh
        ./.factory/tools/check-scratchpad.sh LOOP_COMPLETE
        ./.factory/tools/validate-implementation-plan.py complete .factory/artifacts/implementation-plan.md
        if [[ -x .factory/tools/bug-ledger.py && -f .factory/bugs/open.md ]]; then
...
PY
        ...
        ./.factory/tools/validate-conformance.py complete .factory/artifacts/conformance.json
        ./.factory/tools/validate-blocked-facts.py complete .factory/artifacts/blocked-facts.json
        ./.factory/tools/check-golden-policy.py
        ...
        ./.factory/tools/visual-audit-gate.sh
        ./.factory/tools/check-docs-sync.sh
        ./.factory/tools/verify-boilerplate.sh
        if [[ -x .factory/tools/verify-project.sh ]]; then
        ...
        ;;
    --campaign-audit)
        ./.factory/tools/campaign-audit-scope-guard.sh
        ...
        ./.factory/tools/check-campaign-objectives.py
        ...
        ./.factory/tools/check-scratchpad.sh AUDIT_COMPLETE
        ./.factory/tools/validate-campaign-audit.py complete .factory/artifacts/campaign-audit.md \
        ...
        ;;
    --maintenance)
        ./.factory/tools/validate-maintenance-plan.py complete .factory/artifacts/maintenance-plan.md >/dev/null
        ./.factory/tools/check-maintenance-freshness.sh
        ./.factory/tools/check-scratchpad.sh MAINTENANCE_COMPLETE
        ./.factory/tools/bug-ledger.py validate
        python3 - <<'PY'
import json, re, subprocess
meta = json.loads(subprocess.check_output(
    ['./.factory/tools/validate-maintenance-plan.py', 'metadata', '.factory/artifacts/maintenance-plan.md'], text=True,
))
bug_id, base = meta['bug_id'], meta['base_commit']
shown = subprocess.check_output(['./.factory/tools/bug-ledger.py', 'show', bug_id], text=True)
...
PY
        ...
        ;;
    *)
        echo "Usage: .factory/tools/final-gate.sh --planning|--implementation|--campaign-audit|--maintenance-planning|--maintenance" >&2
        exit 2
        ;;
esac
