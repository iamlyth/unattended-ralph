...
def load_facts(root: Path, path: Path) -> dict:
    try:
        ledger = load_script_module("validate_blocked_facts", root / ".factory/tools/validate-blocked-facts.py")
...
def blocked_facts_module(root: Path):
    ...
    if key not in FACTS_CACHE:
        FACTS_CACHE[key] = load_script_module(
            "validate_blocked_facts", root / ".factory/tools/validate-blocked-facts.py"
        )
...
def check_capability_evidence(root: Path, capabilities: list[str]) -> None:
    try:
        checker = load_script_module("check_capability_evidence", root / ".factory/tools/check-capability-evidence.py")
    except (OSError, UnicodeError) as exc:
