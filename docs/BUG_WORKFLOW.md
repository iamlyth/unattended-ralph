...
maintenance loops (`.factory/tools/ralph-maintenance-plan.sh` /
`.factory/tools/ralph-maintenance-run.sh`) are deprecated forwarders of the removed
Ralph maintenance role; their `--resume`/`--no-tui`/`--continue` options
apply only to that frozen surface. `.factory/tools/verify-project.sh` is
...
forwarders are `./.factory/tools/ralph-maintenance-plan.sh BUG-0001` (planning) and
`./.factory/tools/ralph-maintenance-run.sh` (implementation); review the committed
...
Maintenance fails closed unless `[verification].maintenance_command` is a non-empty argv array whose first element exists and is executable. The generic boilerplate intentionally does not include `.factory/tools/verify-project.sh`; each project must supply that executable before maintenance can complete. The argv is executed directly, without shell evaluation.

Headless and continuation options are `--no-tui` and `--resume`. Recovery
modes are `./.factory/tools/ralph-recover.sh --mode maintenance-planning` and
`./.factory/tools/ralph-recover.sh --mode maintenance`.
