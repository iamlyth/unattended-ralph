...
m=canonical(manifest);policy=canonical(policy_path);transport=canonical(transport_path);launcher=canonical(launcher_path)
...
if None in classes or len(classes)!=len(policy['classes']) or None in accounts or len(accounts)!=len(classes):die('class/account mappings must be unique and configurable')
...
for line in keyraw.splitlines():
 ...
 keys[account]=path
if set(keys)!=accounts:die('exactly one public key is required for every configured account')
required={'scripts/factory-runner-broker.py','scripts/factory-runner-signer.py','scripts/factory-runner-server.py','scripts/factory_runner_policy.py','scripts/factory_runner_authority.py','scripts/factory_runner_artifacts.py'}
...
try:
 ...
 for rel,desc in m['files'].items():
  body=read(source/rel,desc['size']+1)
