...
if [[ ${FACTORY_RALPH_FREEZE_OVERRIDE:-0} != 1 ]]; then
    ...
    if [[ -f "$freeze_marker" && ! -L "$freeze_marker" ]]; then
        echo "ralph-maintenance-run: frozen: the legacy Ralph control plane is deprecated (Task 15 migration)" >&2
        echo "ralph-maintenance-run: the hidden .factory/loop control plane replaces it" >&2
        echo "ralph-maintenance-run: set FACTORY_RALPH_FREEZE_OVERRIDE=1 only to recover an in-flight cycle" >&2
        exit 2
    fi
    if [[ -e "$freeze_marker" || -L "$freeze_marker" ]]; then
        echo "ralph-maintenance-run: frozen: the freeze marker exists but is not a regular file; refusing a legacy launch" >&2
        echo "ralph-maintenance-run: inspect .factory/ralph-freeze; FACTORY_RALPH_FREEZE_OVERRIDE=1 is only for bounded recovery" >&2
        exit 2
    fi
fi
