...
claims = (
    ...
    ('BUG_WORKFLOW project-supplied verifier',
     'adopting-product supplied' in bugs and 'scripts/verify-project.sh' in bugs),
...
verify = (ROOT / 'scripts/verify-boilerplate.sh').read_text(encoding='utf-8')
if 'check-docs-sync.sh' not in verify:
    print('docs-sync: scripts/verify-boilerplate.sh does not run '
