...
def main() -> int:
    ...
    plan_commit = git("log", "-1", "--format=%H", "--", "IMPLEMENTATION_PLAN.md")
    plan_blob = git("rev-parse", "HEAD:IMPLEMENTATION_PLAN.md")
    ...
    for target in (ROOT / "CAMPAIGN_AUDIT.md", ROOT / ".ralph/agent/scratchpad.md"):
...
"""
    atomic_write(ROOT / "CAMPAIGN_AUDIT.md", report)
