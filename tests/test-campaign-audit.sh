...
tmp=$(mktemp -d)
trap 'rm -rf "$tmp"' EXIT
mkdir -p "$tmp/scripts" "$tmp/.ralph/agent"
cp "$PROJECT_ROOT/scripts/initialize-campaign-audit.py" \
   "$PROJECT_ROOT/scripts/validate-campaign-audit.py" "$tmp/scripts/"
chmod +x "$tmp/scripts/"*
...
(cd "$tmp" && ./scripts/initialize-campaign-audit.py --round 1 --base "$base" >/dev/null)
python3 - "$tmp/CAMPAIGN_AUDIT.md" <<'PY'
...
PY
git -C "$tmp" add CAMPAIGN_AUDIT.md .ralph/agent/scratchpad.md
git -C "$tmp" commit -qm pass
(cd "$tmp" && ./scripts/validate-campaign-audit.py complete --expected-round 1 --expected-base "$base" >/dev/null)
set +e
(cd "$tmp" && ./scripts/validate-campaign-audit.py complete --expected-round 2 --expected-base "$base" >/dev/null 2>&1)
binding_rc=$?
set -e
[[ $binding_rc -eq 1 ]]
...
set +e
(cd "$tmp" && ./scripts/validate-campaign-audit.py complete --expected-round 1 --expected-base "$base" >/dev/null 2>&1)
hidden_rc=$?
set -e
[[ $hidden_rc -eq 1 ]]
git -C "$tmp" restore CAMPAIGN_AUDIT.md

# A findings result must provide all remediation fields.
sed -i 's/result: pass/result: findings/; /## Findings/,$c\## Finding 1: gap\n- Requirement: x\n- Production evidence: y' "$tmp/CAMPAIGN_AUDIT.md"
set +e
(cd "$tmp" && ./scripts/validate-campaign-audit.py complete --expected-round 1 --expected-base "$base" >/dev/null 2>&1)
missing_rc=$?
set -e
